<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100">Edit User</h2>
    ...
    <div x-data="{ 
            ...
            selectedImageId: '{{ old('profile_picture_id', $user->profile_picture_id) }}', 
            selectedImagePath: '{{ $user->profilePicture ? asset($user->profilePicture->file_path) : '' }}' 
        }" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1 text-center">
                    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/70 p-6 shadow-sm">
                        ...
                        <button type="button" @click="selectedImageId = ''; selectedImagePath = ''" x-show="selectedImageId" style="display: none;" class="mt-4 text-[10px] font-bold text-rose-500 hover:underline uppercase tracking-wider">Remove Photo</button>
                    </div>
                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/70 p-6 shadow-sm space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900/40 text-gray-800 dark:text-gray-100 focus:ring-blue-500/20">
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900/40 text-gray-800 dark:text-gray-100 focus:ring-blue-500/20">
                        </div>

                        <div class="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl">
                            <p class="text-xs font-bold text-amber-600 dark:text-amber-400 mb-3">Change Password (Leave blank to keep current)</p>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <input type="password" name="password" placeholder="New Password" class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900/40 focus:ring-blue-500/20 text-sm">
                                <input type="password" name="password_confirmation" placeholder="Confirm New Password" class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900/40 focus:ring-blue-500/20 text-sm">
                            </div>
                        </div>

                        <div class="pt-2">
                            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition shadow-md">Update Account Info</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        @forelse($media as $img)
                            <div @click="selectedImageId = '{{ $img->id }}'; selectedImagePath = '{{ asset($img->file_path) }}'; showImageModal = false;" 
                                ...
                                <div x-show="selectedImageId == '{{ $img->id }}'" class="absolute inset-0 border-4 border-blue-500 rounded-xl pointer-events-none" style="display: none;"></div>
