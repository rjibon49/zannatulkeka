...
@php
    ...
    $oldCategories = collect(old('categories', $article->categories->pluck('id')->toArray()))
        ->map(fn ($id) => (string) $id)
        ->toArray();

    $oldTags = collect(old('tags', $article->tags->pluck('id')->toArray()))
        ->map(fn ($id) => (string) $id)
        ->toArray();

    $imageMedia = ($media ?? collect())->where('type', 'image');

    $selectedFeatured = $imageMedia->firstWhere('id', (int) old('featured_media_id', $article->featured_media_id))
        ?: $article->featuredImage;

    $selectedOg = $imageMedia->firstWhere('id', (int) old('og_media_id', $article->og_media_id))
        ?: $article->ogImage;
...
<x-app-layout>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-[#756b62]">
                    Update article content, SEO, category, tags, image and publishing status.
                </p>
            </div>
    ...
    <div
        x-data="{
            ...
            metaDescription: @js(old('meta_description', $article->meta_description)),
            status: @js(old('status', $article->status)),

            categorySearch: '',
            tagSearch: '',

            showFeaturedModal: false,
            selectedFeaturedId: @js(old('featured_media_id', $article->featured_media_id)),
            selectedFeaturedUrl: @js($selectedFeatured?->url),
            selectedFeaturedName: @js($selectedFeatured?->file_name),

            showOgModal: false,
            selectedOgId: @js(old('og_media_id', $article->og_media_id)),
    ...
    >
        <form action="{{ route('articles.update', $article) }}" method="POST" class="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-6 flex items-center gap-3 border-b border-[#784828]/10 pb-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-[#756b62]">
                                Edit title, subtitle, excerpt and main body.
                            </p>
                        </div>
                    </div>
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    <div class="mb-6 flex items-center gap-3 border-b border-[#784828]/10 pb-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-[#756b62]">
                                Update search engine and social sharing metadata.
                            </p>
                        </div>
                    </div>
            ...
            <aside class="space-y-5 xl:sticky xl:top-24 xl:self-start">
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    <div class="mb-5 flex items-center gap-3 border-b border-[#784828]/10 pb-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-[#756b62]">
                                Update article status and schedule.
                            </p>
                        </div>
                    </div>
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    ...
                    <div class="max-h-[280px] space-y-2 overflow-y-auto overscroll-contain rounded-2xl bg-[#fbf7f1] p-3 pr-2 ring-1 ring-[#784828]/10">
                        @forelse($categories as $cat)
                            <label
                                x-show="@js(Str::lower($cat->name . ' ' . $cat->slug)) .includes(categorySearch.toLowerCase())"
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    ...
                    <div class="max-h-[280px] space-y-2 overflow-y-auto overscroll-contain rounded-2xl bg-[#fbf7f1] p-3 pr-2 ring-1 ring-[#784828]/10">
                        @forelse($tags as $tag)
                            <label
                                x-show="@js(Str::lower($tag->name . ' ' . $tag->slug)) .includes(tagSearch.toLowerCase())"
                                class="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2 transition hover:bg-white"
                            >
                                <input
                                    type="checkbox"
