...
@php
    ...
    $selectedImages = $gallery->images
        ->filter(fn ($galleryImage) => $galleryImage->media)
        ->map(fn ($galleryImage) => [
            'id' => (string) $galleryImage->media->id,
            'url' => $galleryImage->media->url,
            'name' => $galleryImage->media->file_name,
            'search' => strtolower(($galleryImage->media->file_name ?? '') . ' ' . ($galleryImage->media->original_name ?? '') . ' ' . ($galleryImage->media->alt_text ?? '')),
        ])
        ->values();

    $selectedCover = $gallery->coverImage ?: $gallery->images->first()?->media;
@endphp
...
<x-app-layout>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-[#756b62]">
                    Update gallery information and selected images.
                </p>
            </div>
    ...
    <div
        x-data="{
            ...
            imageSearch: '',
            selectedImages: @js($selectedImages),
            coverMediaId: @js(old('cover_media_id', $gallery->cover_media_id ?: $selectedCover?->id ?: '')),
            coverUrl: @js($selectedCover?->url ?? ''),
            coverName: @js($selectedCover?->file_name ?? ''),
    ...
    >
        <form action="{{ route('galleries.update', $gallery) }}" method="POST" class="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-6 flex items-center gap-3 border-b border-[#784828]/10 pb-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-[#756b62]">
                                Update title, description, SEO and sorting information.
                            </p>
                        </div>
                    </div>
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    <div class="mb-5 flex flex-col gap-3 border-b border-[#784828]/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm font-medium text-[#756b62]">
                                Change gallery images and cover image.
                            </p>
                        </div>
            ...
            <aside class="space-y-5 xl:sticky xl:top-24 xl:self-start">
                ...
                <div class="rounded-[2rem] border border-[#784828]/10 bg-white/85 p-6 shadow-xl shadow-[#312114]/5">
                    ...
                    <p class="mt-1 text-sm font-medium leading-6 text-[#756b62]">
                        Save changes after editing gallery information or images.
                    </p>
